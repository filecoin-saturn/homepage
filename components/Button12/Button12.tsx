...
export default function Button12({target, text, subtext, link, disabled, contentId}: Props) {
    ...
    const ref = useCallback((node: HTMLElement | null) => {
        node?.setAttribute('data-analytics', a)
      }, [])
    return (
            <a target={target} data-analytics='"blue"' ref={ref} href={l} className={`grow ${disabled ? `pointer-events-none   ` : ``} outline-none group relative  `}>
