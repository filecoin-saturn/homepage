...
export default function Modal3({children, backdropBlur, link}: Props) {
    return (
        <CustomProse overrides='prose-p:my-0 prose-strong:text-white prose-strong:font-semibold prose-p:py-1.5 prose-strong:antialiased prose-p:leading-tight prose-a:no-underline' >
            <Link href={link}>
                <a target="_blank" className={`rounded-[1.625rem] md:rounded-[1.75rem] group pl-6 flex items-center !outline-offset-[-2px] outline-none !outline-2 justify-between pr-2 py-2 space-x-4 bg-sat-white-10-fallback-2 hover:bg-sat-white-20-fallback-2 transition-transform focus-visible:bg-sat-white-30-fallback-2 focus-visible:!outline-white active:scale-95 ${backdropBlur ? "supports-blur:backdrop-blur-md supports-blur:bg-white/10 supports-blur:hover:bg-white/20 supports-blur:focus-visible:bg-white/30" : ""}`}>
                    {children}
